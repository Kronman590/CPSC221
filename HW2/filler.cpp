...
// {
    // customColorPicker a(fillColor, img);
    // return fill<Stack>(img, x, y, a, tolerance, frameFreq);
// }
                       ...
                       double tolerance, int frameFreq)
{
     ...
     *        4.    as you process a neighbor, if it is an appropriate frame, send the current PNG to the
    ...
    animation ani;
    int num_pixels_filled = 0;

    HSLAPixel center = *img.getPixel(x, y);

    //outside vector is y, inside vector is x
    vector<vector<int>> processed_map(img.height(), vector<int>(img.width(), 0)); //nothing is processed yet
    ...
    *img.getPixel(x, y) = fillColor(x, y);
    ...
    num_pixels_filled++;
    if(num_pixels_filled % frameFreq == 0)
        ani.addFrame(img);

    vector<int> first_point;
    first_point.push_back(x);
    first_point.push_back(y);
    OS.add(first_point);

    while(!OS.isEmpty()) {
        vector<int> point = OS.remove();
        int cur_x = point[0];
        int cur_y = point[1];
        int i, j;
        j = cur_y - 1;
        for(i = cur_x + 1; i >= cur_x - 1; i--) {
            // See if the point is inside the image
            if(i >= 0 && i < (int)img.width() && j >= 0 && j < (int)img.height()) {
                // See if the point isn't processed yet AND is within tolerance
                if(processed_map[j][i] == 0 && img.getPixel(i, j)->dist(center) <= tolerance) {
                    *img.getPixel(i, j) = fillColor(i, j);
                    processed_map[j][i] = 1;
...
//basic stripe dfs, bfs, rainbow dfs bfs