...
private:
   ...
   * @param: int k, the dimension of the image 
   * (note that this is redundant--could just be grabbed from the image itself)
   */
   Node * buildTree(PNG * im, int k);
   
   double findEntropyInit(stats stat,int k);
   
   double findEntropy(stats stat,int x,int y,int max);
   
   vector<int> getHist(stats stat,int xCoor,int yCoor,int m);
   
   void makeChild(PNG * NW,PNG * NE,PNG * SW,PNG * SE,PNG org,int x,int y,int sz);
   
   HSLAPixel * getPix(PNG im,int xCoor,int yCoor,int m);
   
   PNG renderHelper(Node *cur_node);
   
   PNG combinePNGs(Node *cur_node, PNG NW_PNG, PNG NE_PNG, PNG SE_PNG, PNG SW_PNG);
   
   int positiveMod(int a, int b);
	   
   void pruneHelper(Node* node, double tol);
   
   bool shouldPrune(Node *subtree_root, Node *node, double tol);
